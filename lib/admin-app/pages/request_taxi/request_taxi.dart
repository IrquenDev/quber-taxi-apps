...
class RequestTravelAdminSheet extends StatefulWidget {
  ...
  @override
  State<RequestTravelAdminSheet> createState() =>
      _RequestTravelAdminSheetState();
}
...
class _RequestTravelAdminSheetState extends State<RequestTravelAdminSheet> {
  ...
  double _getPriceByDistance(double distance) =>
      _taxiPrices[_selectedTaxi]! * distance;
  ...
  bool get _canEstimateWithUnfixedDestination =>
      _usingFixedDestination == false &&
      (_originName != null && _originCoords != null) &&
      (_destinationName != null);

  // It is possible to estimate a distance when any of the above conditions are met
  bool get _canEstimateDistance =>
      _canEstimateWithFixedDestination || _canEstimateWithUnfixedDestination;
  ...
  Future<void> _estimateDistance() async {
    if (_usingFixedDestination) {
      ...
      final entryPoint = GeoUtils.findNearestPointInPolygon(
          benchmark: benchmark, polygon: polygon);
      // Calculate farthest point from entrypoint
      final farthestPoint = GeoUtils.findFarthestPointInPolygon(
  ...
  Widget _buildEstimationInfoRowAdmin(
      TextTheme textTheme, String label, String value) {
